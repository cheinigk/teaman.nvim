...
local function terminal_create_buf(term) rawset(term, "bufnr", vim.api.nvim_create_buf(true, false)) end
...
local function win_invalid(winnr) return not vim.tbl_contains(vim.api.nvim_list_wins(), winnr) end
...
function Terminal:create() require("teaman.utils").with_restore_current_buf(terminal_termopen, self) end
...
function Terminal:open()
  ...
  if not self.chanid then self:create() end
...
function Terminal:toggle()
  local function contains_term(winnr) return vim.api.nvim_win_get_buf(winnr) == self.bufnr end
...
function Terminal:send_motion(motion)
  ...
  if motion == nil then
    ...
    require("teaman.utils").set_opfunc(function(m) self:send_motion(m) end)
    return "g@"
  end
...
local terminal_mt = {
  __index = Terminal,
  __newindex = function(_, _, _) error "The Terminal table is constant." end,
  __tostring = function(term)
    local fmt = "Terminal{shell=%q, bufnr=%d, winnr=%d, chanid=%d}"
    local str =
      string.format(fmt, table.concat(term.config.shell) or "", term.bufnr or -1, term.winnr or -1, term.chanid or -1)
    return str
  end,
}
