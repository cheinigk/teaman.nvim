...
local function tbl_difference(t1, t2)
  ...
  for _, entry in ipairs(t1) do
    if not vim.tbl_contains(t2, entry) then table.insert(res, entry) end
  end
  return res
end
  ...
  describe("create a terminal object with default config:", function()
    ...
    it(
      ...
      function()
        eq(
          tostring(terminal),
          [[Terminal{shell="]] .. vim.o.shell .. [[", bufnr=-1, winnr=-1, chanid=-1}]]
        )
      end
    )
    describe("call create()", function()
      local bufnrs = vim.api.nvim_list_bufs()
      terminal:create()
      it("creates a new buffer and a terminal represented by a channel id", function()
        ...
        local chanids = vim.tbl_map(function(chan_info) return chan_info.id end, chan_infos)
        eq(vim.tbl_contains(chanids, terminal.chanid), true)
      end)
    end)
    describe("call open()", function()
      local winnrs = vim.api.nvim_list_wins()
      terminal:open()
      it("creates a new window with the terminal buffer attached", function()
